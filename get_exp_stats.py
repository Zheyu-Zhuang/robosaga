...
def format_mean_std(
    ...
    if include_std:
        std_str = f" \\pm {std:.{precision}f}"
    else:
        std_str = ""
    if bold:
        return f"$\\boldsymbol{{ {mean_str}{std_str}}}$"
...
def print_latex_tables(stats_dict, print_std=False):
    ...
    \\footnotesize % Makes table font smaller
    \\centering
    \\setlength{\\tabcolsep}{4pt} % Adjust column spacing
        \\centering
        \\begin{tabular}{cl cccc cccc cccc}
            \\toprule
            \\textit{Policy} & & \\multicolumn{4}{c}{\\textbf{BC}} & \\multicolumn{4}{c}{\\textbf{BC-RNN}} & \\multicolumn{4}{c}{\\textbf{Diffusion Policy}} \\\\
                \\cmidrule(lr){3-6} \\cmidrule(lr){7-10} \\cmidrule(lr){11-14}
            \\textit{Method} & & \\small{\\textit{None}} & \\small{\\textit{Over.}} & \\small{\\textit{SO}} & \\small{\\textit{Sa}} & \\small{\\textit{None}} & \\small{\\textit{Over.}} & \\small{\\textit{SO}} & \\small{\\textit{Sa}} & \\small{\\textit{None}} & \\small{\\textit{Over.}} & \\small{\\textit{SO}} & \\small{\\textit{Sa}}  \\\\
            \\midrule
            \\textit{Task} & \\textit{Domain} & &&&& &&&& &&& \\\\
            \\midrule
"""
...
if __name__ == "__main__":
    ...
    if args.format == "latex":
        ...
        output = print_latex_tables(stats, print_std=False)
