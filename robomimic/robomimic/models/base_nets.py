...
# SaGA Modification: replace in-place ReLU and out += identity.
from robosaga.resnet import ResNet18_Weights, resnet18
...
class R3MConv(ConvBase):
    ...
    def __init__(
        ...
        self.nets = Sequential(
            *([preprocess] + list(net.module.convnet.children())), has_output_shape=False
        )
        if freeze:
            self.nets.freeze()
...
class CoordConv2d(nn.Conv2d, Module):
    ...
    def __init__(
        ...
        if coord_encoding == "position":
            ...
            raise Exception(
                "CoordConv2d: coord encoding {} not implemented".format(self.coord_encoding)
            )
        nn.Conv2d.__init__(
            self,
...
class Conv1dBase(Module):
    ...
    def output_shape(self, input_shape):
        ...
        for i in range(self.n_layers):
            ...
            length = (
                int(
                    (length + 2 * net.padding[0] - net.dilation[0] * (net.kernel_size[0] - 1) - 1)
                    / net.stride[0]
                )
                + 1
            )
