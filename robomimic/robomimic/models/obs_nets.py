...
from robomimic.models.obs_core import Randomizer, VisualCore
...
def obs_encoder_factory(
    ...
    for k, obs_shape in obs_shapes.items():
        ...
        for obs_module, cls_mapping in zip(
            ("core", "obs_randomizer"), (ObsUtils.OBS_ENCODER_CORES, ObsUtils.OBS_RANDOMIZERS)
        ):
    ...
    def forward(self, obs_dict):
        ...
        for k in self.obs_shapes:
            ...
            if self.obs_randomizers[k] is not None:
                if self.obs_randomizers[k].output_shape_in(x.shape) != list(x.shape[1:]):
